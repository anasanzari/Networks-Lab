...
void main(){
  ...
  //create tld data
  struct resource records[10];
	int recordslen = 0;
  records[recordslen++] = createresource("com","dns.com",Q_NS,2500);
  records[recordslen++] = createresource("dns.com","127.0.0.10",Q_A,2500);
  ...
  struct msg m;
	while(1){
	  ...
	  if(m.msg_type==MSG_QUERY){
	    ...
	    char* n = m.queries[0].name;
	    char* tldname = n+strlen(n)-3;
	    printf("Query for %s recieved.\n",tldname);

	    struct resource found[4];
			int l = search(records,recordslen,found,tldname);

			if(l>0){
				m.answers[0] = found[0];
				struct resource dns[4];
				int k = search(records,recordslen,dns,found[0].value);
				m.answers[1] = dns[0];
