...
void main(){
  ...
  //create tld data
  struct resource records[50];
  int recordslen = 0;
  records[recordslen++] = createresource("athena.com","200.50.100.11",Q_A,3600);
  records[recordslen++] = createresource("www.athena.com","athena.com",Q_CNAME,3600);
  records[recordslen++] = createresource("mail.athena.com","athena.com",Q_MX,3600);

  records[recordslen++] = createresource("facebook.com","200.50.100.11",Q_A,3000);
  records[recordslen++] = createresource("www.facebook.com","facebook.com",Q_CNAME,3000);
  records[recordslen++] = createresource("mail.facebook.com","facebook.com",Q_MX,3000);

  records[recordslen++] = createresource("google.com","70.50.99.11",Q_A,3600);
  records[recordslen++] = createresource("www.google.com","google.com",Q_CNAME,3600);
  records[recordslen++] = createresource("mail.google.com","google.com",Q_MX,3600);

  records[recordslen++] = createresource("amazon.com","10.60.100.11",Q_A,3000);
  records[recordslen++] = createresource("www.amazon.com","amazon.com",Q_CNAME,3000);
  records[recordslen++] = createresource("mail.amazon.com","amazon.com",Q_MX,3000);
      ...
      if(m.msg_type==MSG_QUERY){
        ...
        if(k>0){
          ...
          if(found[0].type != Q_A){
            ...
            struct resource arec[4];
            int l = search(records,recordslen,arec,found[0].value);
            ...
            if(l>0){
              ...
              int i;
              for (i = 0; i < l; i++) {
                m.answers[1+i] = arec[i];
              }
              printRecords(1+1, m.answers);

              sendquery(sockfd,m,MSG_REPLY,0,1+l,clientaddress);
            ...
            printRecords(k, found);
            sendquery(sockfd,m,MSG_REPLY,0,k,clientaddress);
